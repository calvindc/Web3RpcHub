...
func (r *aliasR) GetMember() *Member {
	if r == nil {
		return nil
	}
	return r.Member
...
var (
	...
	aliasGeneratedColumns      = []string{}
)

type (
	// AliasSlice is an alias for a slice of pointers to Alias.
	// This should almost always be used instead of []Alias.
	AliasSlice []*Alias
...
func (aliasL) LoadMember(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAlias interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAlias.(*Alias)
		if !ok {
			object = new(Alias)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAlias)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAlias))
			}
		}
	} else {
		s, ok := maybeAlias.(*[]*Alias)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAlias)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAlias))
			}
		}
	}
...
func Aliases(mods ...qm.QueryMod) aliasQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"aliases\".*"})
	}
...
func FindAlias(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Alias, error) {
	...
	if err = aliasObj.doAfterSelectHooks(ctx, exec); err != nil {
		return aliasObj, err
	}
...
// Exists checks if the Alias row exists.
func (o *Alias) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AliasExists(ctx, exec, o.ID)
}
...
func (o *Alias) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no aliases provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert aliases, could not build update column list")
		}
		...
		cache.query = buildUpsertQuerySQLite(dialect, "\"aliases\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert aliases")
	}
