...
func (i Invites) Consume(ctx context.Context, token string, newMember refs.FeedRef) (db.Invite, error) {
	...
	err = transact(i.db, func(tx *sql.Tx) error {
		...
		_, err = i.members.add(ctx, tx, newMember, db.RoleMember)
		var alreadyAdded db.ErrAlreadyAdded
		if err != nil {
		...
		inv.CreatedBy.Role = db.Role(entry.R.CreatedByMember.Role)
...
func (i Invites) GetByToken(ctx context.Context, token string) (db.Invite, error) {
	var inv db.Invite
...
func (i Invites) GetByID(ctx context.Context, id int64) (db.Invite, error) {
	var inv db.Invite
...
func (i Invites) List(ctx context.Context) ([]db.Invite, error) {
	var invs []db.Invite
	...
	err := transact(i.db, func(tx *sql.Tx) error {
		...
		invs = make([]db.Invite, len(entries))
