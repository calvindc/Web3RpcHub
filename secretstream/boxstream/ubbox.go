...
func (u *Unboxer) ReadMessage() ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("[secretstream/box] invalid header box")
	}
	...
	if bodyLen > MaxSegmentSize {
		return nil, errors.New("[secretstream/box] message exceeds maximum segment size")
	}
	...
	if !ok {
		return nil, errors.New("[secretstream/box] invalid body box")
	}
	return msg, nil
}
